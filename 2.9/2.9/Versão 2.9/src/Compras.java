...
public class Compras {
   ...
   /* public boolean comprar_passagem(Trecho trecho, Data hora_compra, int quantCompradas_Pass) {
    ...
    private double UpdateReturnSaldo(double valor) { //Atualiza saldo e retorna o valor gasto automaticamente. pra n ter q ficar refazendo linha
        cliente.setSaldo(cliente.getSaldo() - valor);
        return cliente.getSaldo() + valor;
    }

    private double Comprar_Quarto(Hotel hotel, int QuantCompras, int indice, Data d1, Trecho trecho) {
        double retorno = 0;
            ...
            switch (indice) {
                    case 1: if (Tem_Suficiente(saldoc, preco1)) {
                        ...
                        break; 
                            ...
                            historico_pesquisa.AddHistorico(d1, trecho); 
                            break; 
                        }
                        
                    }
                    case 2: if (Tem_Suficiente(saldoc, preco2) ) {
                        retorno = UpdateReturnSaldo(preco2); break; 
                    }

                    case 3: if (Tem_Suficiente(saldoc, preco3)) {
                        retorno = UpdateReturnSaldo(preco3); break;
                    }
                    case 4: if (Tem_Suficiente(saldoc, luxo)) {
                        retorno = UpdateReturnSaldo(luxo); break;
                    }
            }   
        }
        else { //ele e vip
            double temp = cliente.getVip().getDesconto();
            switch (indice) {
                case 1: if (Tem_Suficiente(saldoc, preco1 * temp)) {
                    retorno = UpdateReturnSaldo(preco1 * temp); break;
                }
                case 2: if (Tem_Suficiente(saldoc, preco2 * temp)) {
                    retorno = UpdateReturnSaldo(preco2 * temp); break;
                }
                case 3: if (Tem_Suficiente(saldoc, preco3 * temp)) {
                    retorno = UpdateReturnSaldo(preco3 * temp); break;
                }
                case 4: if (Tem_Suficiente(saldoc, temp * luxo)) {
                    retorno = UpdateReturnSaldo(temp * luxo); break;
                }
            }
        }
    ...
    public void Realizar_Compra(Trecho trecho, PassagemAerea passagem, Data Hora_Compra, int QuantCompras,Hotel hotel, int QuantCompras2, String metodo, int indice) {
        double soma_total = Comprar_Passagem(trecho, Hora_Compra, QuantCompras) + Comprar_Quarto(hotel, QuantCompras2, indice, Hora_Compra, trecho);
        System.out.printf("\nValor total gasto:\t %2f\n", soma_total);

        double aux = trecho.getPreco();
        double aux2;
        switch (indice) {
            case 1: aux2 = hotel.getQuartosHotel().getPrecoQuartoSingle(); break;
            case 2: aux2 = hotel.getQuartosHotel().getQntQuartosDuplos(); break;
            case 3: aux2 = hotel.getQuartosHotel().getPrecoQuartoTriplo(); break;
            case 4: aux2 = hotel.getQuartosHotel().getPrecoQuartoLuxo(); break;
            default : aux2 = 0; break;
        }
        historica_compra = new Historico(hotel,passagem,aux, aux2, metodo, QuantCompras, QuantCompras2);
