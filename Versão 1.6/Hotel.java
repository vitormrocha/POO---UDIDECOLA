...
public class Hotel extends Empresa{
    ...
    public void menu() {
        for (ProdutosOferecidos produto : produtos)
            System.out.printf("\nNOME PRODUTO:\t%s\nPRECO:\t%2f\nQUANTIA DISPONIVEL:\t%d", produto.getNome_produto(), produto.getPreco(), produto.getQuant());
    }
    ...
    public boolean realizar_compra (int opcao, int num, Cliente cliente) {
        if (opcao > produtos.size() || opcao < 0 || num > produtos.get(opcao).getQuant() || num <= 0) 
            return false;
        else if (cliente.getSaldo() < produtos.get(opcao).getPreco()) {
            System.out.println("\nSALDO INSUFICIENTE.\n");
            return false;
        }
        else {
            cliente.pagar(produtos.get(opcao).getPreco());
